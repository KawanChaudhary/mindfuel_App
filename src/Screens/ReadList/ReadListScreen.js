...
const ReadListScreen = () => {
    ...
    const onPressBookmark = async (storySlug) => {
        if (auth === false) {
            ...
            try {
                const config = await setUserConfig();
                await axios.post(`/user/${storySlug}/addStoryToReadList`, { activeUser }, config);
    ...
    const renderStory = ({ item }) => (
        <ListSkeleton loading={loading} theme={theme} story={item} navigation={navigation} onPressBookmark={onPressBookmark} />
    );
        ...
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            ...
            <SafeAreaView>
                ...
                <ScrollView>
                    ...
                    {/* ReadList Text */}
                    <Text style={[styles.readListText, { color: theme.colors.text }]}>{!loading && 'Reading List'}</Text>
