...
const DetailStoryScreen = ({ route }) => {
    ...
    const getDetailStory = useCallback(async () => {
        setLoading(true);
        try {
            const { data } = await axiosInstance.post(`/story/${storySlug}`, { activeUser });
            setStory(data.data);
    ...
    const onPressLike = async () => {
        ...
        if (auth === false) {
            ...
            try {
                const { data } = await axiosInstance.post(`/story/${storySlug}/like`, { activeUser }, config);
                setLikeCount(data.data.likeCount);
            }
            catch (error) {
                setStory({});
    ...
    const onPressBookmark = async () => {
        if (auth === false) {
            ...
            try {
                const config = await setUserConfig();
                const { data } = await axiosInstance.post(`/user/${storySlug}/addStoryToReadList`, { activeUser }, config);
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.colors.backgroundLight }]}>
            <CustomizeStatusBar />
            <Header title="Story" showBackButton={true} />
            {loading ? <Loader />
                : (
                    <>
                        <ScrollView>

                            <UserInfoBar theme={theme} story={story} activeUser={activeUser} setUserConfig={setUserConfig}
                                navigation={navigation} openEditStoryModal={openEditStoryModal}
                            />
                            <View style={[styles.detailsContainer, { backgroundColor: theme.colors.background }]}>
                                <StoryInfoBar theme={theme} createdAt={story.createdAt} readTime={story.readtime} iconSize={20} />

                                <Text style={[styles.titleText, { color: theme.colors.text }]}>{story.title}</Text>
                                <Image source={{ uri: story.image }} style={styles.image} />
                                <RenderHTML contentWidth={width} source={{ html: story.content }} tagsStyles={contentStyle} />
                            </View>

                        </ScrollView>

                        <Interaction theme={theme} likeStatus={likeStatus} likes={likeCount} comments={commentCount} bookmarked={storyReadListStatus}
                            onPressBookmark={onPressBookmark} onPressComment={onPressComment} onPressLike={onPressLike}
                        />
                        <CommentPopUp visible={CommentModalVisible} handleClose={setCommentModalVisible} storySlug={story.slug} theme={theme}
                            auth={auth} setUserConfig={setUserConfig} navigation={navigation} activeUser={activeUser} />

                        <EditStoryModal visible={editStoryModal} closeModal={setEditStoryModal} story={story} theme={theme}
                            setUserConfig={setUserConfig} getDetailStory={getDetailStory} />
